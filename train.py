...
def main():
  ...
  '''
  A = tfds.load(name = 'cycle_gan/horse2zebra', split = "trainA", download = False).map(parse_function).shuffle(batch_size).batch(batch_size);
  B = tfds.load(name = 'cycle_gan/horse2zebra', split = "trainB", download = False).map(parse_function).shuffle(batch_size).batch(batch_size);
